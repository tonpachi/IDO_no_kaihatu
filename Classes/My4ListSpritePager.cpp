...
My4ListSpritePager::My4ListSpritePager(){
...
//初期化
bool My4ListSpritePager::init(){
    CCLOG("My4ListSpritePager::init");
    if (CCLayer::init()) {
        // タッチ有効化
...
#pragma mark 初期設定
void My4ListSpritePager::prepere(int wsize,int hsize,CCPoint _position){
...
void My4ListSpritePager::addSp(CCSprite* sp){
    sp->setAnchorPoint(CCPointZero);
    
    int _spw=sp->getContentSize().width;
    
	sp->setPosition(ccp(m_p_last.x+(m_content_w-_spw)/2,m_p_last.y));
    //rootSpにaddする
	rootSp->addChild(sp);
    //管理配列にもadd
    spArray->addObject(sp);
...
void My4ListSpritePager::addSpByFilename(const char* pngname){
    //テクスチャアトラス使う前提
    //spriteをaddChild
...
void My4ListSpritePager::ccTouchMoved(CCTouch *pTouch, CCEvent *pEvent){
    
}

bool My4ListSpritePager::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent) {
    CCLOG("My4ListSpritePager::ccTouchBegan");
...
void My4ListSpritePager::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent) {
    CCLOG("My4ListSpritePager::ccTouchEnded");
	...
	if (m_flick_w <= _movew) {
        CCCallFunc *actionDone = CCCallFunc::create(this, callfunc_selector(My4ListSpritePager::onAnimatoinFinished));
...
void My4ListSpritePager::ccTouchCancelled(CCTouch *pTouch, CCEvent* pEvent){
    
}

#pragma mark アニメーション終わった
void My4ListSpritePager::onAnimatoinFinished(){
    canTouch=true;
...
int My4ListSpritePager::myGetNowShowing(){
    return nowContentNum;
}
void My4ListSpritePager::mySetFlickWidth(int wid){
    m_flick_w=wid;
}
int My4ListSpritePager::myGetContentNum(){
    return contentsNum;
}
void My4ListSpritePager::mySetShowPage(int n){
...
void My4ListSpritePager::mySetLastPage(int n){
    lastPageNum=n;
}


void My4ListSpritePager::myInitButton(const char* l_pngname,CCPoint l_position,const char* r_pngname,CCPoint r_position){
    
    leftBtn = MyEasyMenuItem::create(l_pngname, l_position, this, menu_selector(My4ListSpritePager::onLeft));
    
    rightBtn = MyEasyMenuItem::create(r_pngname, r_position, this, menu_selector(My4ListSpritePager::onRight));
...
void My4ListSpritePager::onLeft(CCObject *pSender){
    if (!canTouch) return;
    //右へ動かす　つまり左が出る
    if(nowContentNum<=0)return;
    CCLOG("migi");
...
void My4ListSpritePager::onRight(CCObject *pSender){
    if (!canTouch) return;
    //左へ動かす　つまり右が出る
    if(nowContentNum==contentsNum-1)return;
    CCLOG("hidari");
