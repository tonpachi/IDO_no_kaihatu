...
//初期化
bool MySpritePager::init(){
    CCLOG("MySpritePager::init");
    if (CCLayer::init()) {
        // タッチ有効化
...
#pragma mark 初期設定
void MySpritePager::prepere(int wsize,int hsize,CCPoint _position){
    ...
    this->addChild(m_clippingNode,kpageroot);
    nowContentNum=0;
    m_rootY=0;
...
void MySpritePager::addSpByFilename(const char* pngname){
    //テクスチャアトラス使う前提
    //spriteをaddChild
...
void MySpritePager::ccTouchMoved(CCTouch *pTouch, CCEvent *pEvent){
    
}

bool MySpritePager::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent) {
    CCLOG("MySpritePager::ccTouchBegan");
...
void MySpritePager::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent) {
    CCLOG("MySpritePager::ccTouchEnded");
	...
	if (m_flick_w <= _movew) {
        CCCallFunc *actionDone = CCCallFunc::create(this, callfunc_selector(MySpritePager::onAnimatoinFinished));
...
void MySpritePager::ccTouchCancelled(CCTouch *pTouch, CCEvent* pEvent){
    
}

#pragma mark アニメーション終わった
void MySpritePager::onAnimatoinFinished(){
    canTouch=true;
...
int MySpritePager::myGetNowShowing(){
    return nowContentNum;
}
void MySpritePager::mySetFlickWidth(int wid){
    m_flick_w=wid;
}
int MySpritePager::myGetContentNum(){
    return contentsNum;
}
void MySpritePager::mySetShowPage(int n){
...
void MySpritePager::mySetLastPage(int n){
    lastPageNum=n;
}


void MySpritePager::myInitButton(const char* l_pngname,CCPoint l_position,const char* r_pngname,CCPoint r_position){
    
    leftBtn = MyEasyMenuItem::create(l_pngname, l_position, this, menu_selector(MySpritePager::onLeft));
    
    rightBtn = MyEasyMenuItem::create(r_pngname, r_position, this, menu_selector(MySpritePager::onRight));
    
	CCMenu *menuS = CCMenu::create(leftBtn, rightBtn,NULL);
	menuS->setPosition(ccp(0, 0));
    
	this->addChild(menuS,kpagebtn);
...
void MySpritePager::onLeft(CCObject *pSender){
    if (!canTouch) return;
    //右へ動かす　つまり左が出る
    if(nowContentNum<=0)return;
    CCLOG("migi");
...
void MySpritePager::onRight(CCObject *pSender){
    if (!canTouch) return;
    //左へ動かす　つまり右が出る
    if(nowContentNum==contentsNum-1)return;
    CCLOG("hidari");
