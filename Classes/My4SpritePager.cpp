...
//初期化
bool My4SpritePager::init(){
    CCLOG("My4SpritePager::init");
    if (CCLayer::init()) {
        // タッチ有効化
...
#pragma mark 初期設定
void My4SpritePager::prepere(int wsize,int hsize,CCPoint _position){
    ...
    //閉じるボタンもadd
    MyEasyMenuItem *_backItem = MyEasyMenuItem::create("ok_btn.png", ccp(320,200), this, menu_selector(My4SpritePager::closeAddLayer));
    
    CCMenu *_menuS = CCMenu::create(_backItem,NULL);
	_menuS->setPosition(ccp(0, 0));
    _menuS->setTouchPriority(kModalLayerPriority_a);
    this->addChild(_menuS);
}


#pragma mark add_array
void My4SpritePager::addSpByFilename(const char* pngname){
    //テクスチャアトラス使う前提
    //spriteをaddChild
...
void My4SpritePager::ccTouchMoved(CCTouch *pTouch, CCEvent *pEvent){
    
}

bool My4SpritePager::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent) {
    CCLOG("My4SpritePager::ccTouchBegan");
...
void My4SpritePager::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent) {
    CCLOG("My4SpritePager::ccTouchEnded");
	...
	if (m_flick_w <= _movew) {
        CCCallFunc *actionDone = CCCallFunc::create(this, callfunc_selector(My4SpritePager::onAnimatoinFinished));
...
void My4SpritePager::ccTouchCancelled(CCTouch *pTouch, CCEvent* pEvent){
    
}

#pragma mark アニメーション終わった
void My4SpritePager::onAnimatoinFinished(){
    canTouch=true;
...
int My4SpritePager::myGetNowShowing(){
    return nowContentNum;
}
void My4SpritePager::mySetFlickWidth(int wid){
    m_flick_w=wid;
}
int My4SpritePager::myGetContentNum(){
    return contentsNum;
}
void My4SpritePager::mySetShowPage(int n){
...
void My4SpritePager::mySetLastPage(int n){
    lastPageNum=n;
}


void My4SpritePager::myInitButton(const char* l_pngname,CCPoint l_position,const char* r_pngname,CCPoint r_position){
    
    leftBtn = MyEasyMenuItem::create(l_pngname, l_position, this, menu_selector(My4SpritePager::onLeft));
    
    rightBtn = MyEasyMenuItem::create(r_pngname, r_position, this, menu_selector(My4SpritePager::onRight));
...
void My4SpritePager::onLeft(CCObject *pSender){
    if (!canTouch) return;
    //右へ動かす　つまり左が出る
    if(nowContentNum<=0)return;
    CCLOG("migi");
...
void My4SpritePager::onRight(CCObject *pSender){
    if (!canTouch) return;
    //左へ動かす　つまり右が出る
    if(nowContentNum==contentsNum-1)return;
    CCLOG("hidari");
...
void My4SpritePager::closeAddLayer(CCObject *pSender){
